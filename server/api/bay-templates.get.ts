...
export default defineEventHandler(async (event) => {
  ...
  const templates = await db.select().from(bayTemplates)
    .where(eq(bayTemplates.isArchived, false))
    .orderBy(asc(bayTemplates.name))
  const rows = await db.select().from(bayTemplateRows)
  ...
  return templates.map(template => ({
    ...
    rows: rows.filter(row => row.templateId === template.id).map(row => ({
