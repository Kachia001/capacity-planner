...
const isMobileRole = computed(() => props.role === 'manager' || props.role === 'worker')
const workGroupCount = computed(
  () =>
    new Set(props.items.map(item => (item.workName?.trim() || '작업명 없음').toLocaleLowerCase()))
      .size,
)
...
<template>
  <section
    ...
    :class="isMobileRole ? 'px-0 py-0 sm:px-5 sm:py-6' : 'px-4 py-7 sm:px-6 lg:px-8'"
  >
    <div
      class="overflow-hidden border border-zinc-300 bg-white"
      :class="
        isMobileRole
          ? 'rounded-none border-x-0 shadow-none sm:rounded-2xl sm:border-x sm:shadow-[0_18px_55px_-42px_rgba(15,23,42,0.45)]'
          : 'rounded-lg shadow-[0_22px_70px_-50px_rgba(15,23,42,0.55)]'
      "
    >
      <header
        class="border-b border-zinc-200 bg-[#f8faf7] px-4 sm:px-6"
        :class="isMobileRole ? 'py-4' : 'py-5'"
      >
        <div class="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
          <div>
            ...
            <h2
              class="mt-2 font-semibold tracking-[-0.025em] text-zinc-950"
              :class="isMobileRole ? 'text-xl' : 'text-2xl'"
            >
              {{ props.title }}
            </h2>
        ...
        <div
          class="grid gap-3 xl:grid-cols-[14rem_minmax(18rem,1fr)_11rem_auto]"
          :class="isMobileRole ? 'mt-4' : 'mt-5'"
        >
          <label class="relative">
            <span class="sr-only">Bay 선택</span>
            <select
              :value="props.selectedBayId ?? ''"
              class="w-full appearance-none rounded-md border border-zinc-300 bg-white px-3 pr-9 text-sm font-semibold text-zinc-900 outline-none transition focus:border-emerald-600 focus:ring-2 focus:ring-emerald-100"
              :class="isMobileRole ? 'h-12' : 'h-11'"
          ...
          <label class="relative">
            ...
            <input
              ...
              class="w-full rounded-md border border-zinc-300 bg-white pl-10 pr-3 text-sm outline-none transition placeholder:text-zinc-400 focus:border-emerald-600 focus:ring-2 focus:ring-emerald-100 disabled:cursor-not-allowed disabled:bg-zinc-100"
              :class="isMobileRole ? 'h-12' : 'h-11'"
          ...
          <label class="relative">
            ...
            <select
              ...
              :disabled="!props.selectedBayId"
              class="w-full appearance-none rounded-md border border-zinc-300 bg-white px-3 pr-9 text-sm font-semibold outline-none focus:border-emerald-600 focus:ring-2 focus:ring-emerald-100 disabled:bg-zinc-100"
              :class="isMobileRole ? 'h-12' : 'h-11'"
      ...
      <div
        v-else-if="props.pending && props.items.length === 0"
      ...
      <div v-else :class="isMobileRole ? 'p-3 sm:p-6' : 'p-4 sm:p-6'">
        <div
          class="mb-4 flex flex-wrap items-center justify-between gap-3 border-b border-zinc-200 pb-4"
        >
          <p class="text-sm text-zinc-600">
            <template v-if="isMobileRole">
              <strong class="font-semibold text-zinc-950">{{ workGroupCount }}</strong
              >개 작업 · 세부 <strong class="font-semibold text-zinc-950">{{ props.total }}</strong
              >건
            </template>
            <template v-else>
            ...
            >
              ...
              <div class="flex flex-1 flex-col p-4">
                <div>
                  ...
                  <h3
                    class="mt-1 text-lg font-semibold leading-6 tracking-[-0.015em] text-zinc-950"
                  >
