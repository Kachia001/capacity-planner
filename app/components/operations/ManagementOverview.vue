...
const filteredBays = computed(() => {
  const query = bayQuery.value.trim().toLocaleLowerCase()
  const bays = props.dashboard?.bays ?? []
  ...
  return bays
    .filter(bay => {
      ...
      if (bayViewFilter.value === 'attention') {
        return bay.openIssues > 0
      }

      if (bayViewFilter.value === 'in_progress') {
        return bay.inProgress > 0
      }

      if (bayViewFilter.value === 'completed') {
        return bay.total > 0 && bay.completed === bay.total
      }

      if (bayViewFilter.value === 'high_altitude') {
        return bay.highAltitude > 0
      }
...
function selectBay(bayId: string) {
...
<template>
  <section class="border-b border-zinc-300 bg-[#edf2ee]">
    <div class="mx-auto w-full max-w-[94rem] px-4 py-8 sm:px-6 lg:px-8">
        ...
        <div class="relative flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between">
          <div>
            ...
            <p class="mt-3 max-w-2xl text-sm leading-6 text-zinc-300">
              미작업, 작업 중, 완료 분포와 이슈·고소작업을 한 화면에서 확인하고 필요한 Bay로 바로
              이동합니다.
            </p>
          </div>
      ...
      <section class="mt-5 overflow-hidden rounded-lg border border-zinc-300 bg-white shadow-sm">
        <div
          class="flex flex-col gap-4 border-b border-zinc-200 bg-[#f9faf8] px-4 py-4 lg:flex-row lg:items-center lg:justify-between sm:px-5"
        >
          <div>
            <p class="font-mono text-[10px] font-bold uppercase tracking-[0.2em] text-emerald-700">
              Bay matrix
            </p>
            <h2 class="mt-1 text-lg font-semibold text-zinc-950">Bay별 상태 분포</h2>
          </div>
        ...
        <div
          v-if="props.pending && !props.dashboard"
          class="flex min-h-64 items-center justify-center text-sm text-zinc-500"
        >
          <Loader2 class="mr-2 size-5 animate-spin text-emerald-600" /> 운영 집계를 불러오는
          중입니다.
        </div>
        ...
        <div v-else class="grid gap-3 p-4 sm:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-4">
          <button
            ...
            :key="bay.id"
            type="button"
            class="group rounded-md border p-4 text-left transition duration-200 hover:-translate-y-0.5 hover:border-emerald-400 hover:shadow-md focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-emerald-500"
            :class="
          ...
          >
            <div class="flex items-start justify-between gap-3">
              ...
              <ArrowUpRight
                class="size-4 shrink-0 text-zinc-400 transition group-hover:translate-x-0.5 group-hover:-translate-y-0.5 group-hover:text-emerald-700"
              />
            </div>
            ...
            >
              <span
                class="bg-zinc-300"
                :style="{ width: segmentWidth(bay.notStarted, bay.total) }"
              />
              <span
                class="bg-amber-400"
                :style="{ width: segmentWidth(bay.inProgress, bay.total) }"
              />
              <span
                class="bg-emerald-500"
                :style="{ width: segmentWidth(bay.completed, bay.total) }"
              />
            </div>
