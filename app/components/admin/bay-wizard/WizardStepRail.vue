...
<template>
  <nav
    class="overflow-hidden rounded-md border border-zinc-300 bg-[#f7f9f6] shadow-sm"
    aria-label="BAY 생성 진행 상황"
  >
    <div class="flex items-center justify-between gap-4 border-b border-zinc-300 px-4 py-3 sm:px-5">
      <div class="flex items-center gap-3">
        <p class="font-mono text-[10px] font-bold uppercase tracking-[0.22em] text-zinc-500">
    ...
    <ol class="grid grid-cols-4">
      <li
        ...
        :key="step.number"
        class="relative min-w-0 border-r border-zinc-200 last:border-r-0"
      >
        <Button
          type="button"
          variant="ghost"
          tone="neutral"
          size="content"
          class="group h-full min-h-[4.75rem] w-full justify-start gap-2.5 whitespace-normal rounded-none px-3 py-3 text-left sm:gap-3 sm:px-4"
          :class="
        ...
        >
          ...
          <span class="min-w-0">
            <span
              class="hidden font-mono text-[9px] font-bold uppercase tracking-[0.16em] opacity-60 md:block"
              >{{ step.eyebrow }}</span
            >
            <span class="block truncate text-[11px] font-semibold sm:text-sm">{{
              step.label
            }}</span>
            <span class="mt-0.5 hidden truncate text-[11px] opacity-60 xl:block">{{
              step.description
            }}</span>
          </span>
        </Button>
